...
import { ChevronLeft, ChevronRight, ChevronUp, Edit, MapPinIcon, Menu, Trash2, UserPlusIcon, X, Bell, Users, Download, LogOut } from "lucide-react";
    ...
    <div className="flex space-x-4">
      ...
      <div className="w-3/5 space-y-3">
        ...
        <div className="flex flex-row gap-2">
          ...
          <Textarea className="h-32" placeholder="説明を追加" value={description} onChange={(e) => setDescription(e.target.value)} />
        </div>
...
export default function Home() {
   ...
   */
  const menuItems = [
    { icon: Bell, label: "通知", onClick: () => setIsNotificationsOpen(!isNotificationsOpen) },
    { icon: Users, label: "友達", onClick: () => console.log("友達") },
    { icon: Download, label: "インポート", onClick: () => console.log("インポート") },
    { icon: LogOut, label: "ログアウト", onClick: () => console.log("ログアウト") },
  ];
  ...
  const updateStickyNote = (updatedNote: StickyNote) => {
    const updatedNotes = stickyNotes.map((note) => (note.id === updatedNote.id ? updatedNote : note));
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        ...
        {Array.from({ length: weeks }).map((_, weekIndex) => {
          ...
          const maxEventsInWeek = Math.max(...weekDays.map((day) => getTodoCountForDay(day) + getEventCountForDay(day)));
                    ...
                    {(todoCount > 0 || eventCount > 0) && (
                      <div className="mt-1 space-y-1">
                        {dayItems.slice(0, 2).map((item, index) => (
                          <div key={index} className={`text-xs p-1 rounded ${"text" in item ? priorityColors[item.priority] : priorityColors[item.priority]}`}>
                            {"text" in item ? item.text : item.title}
                          </div>
                        ))}
                        {dayItems.length > 2 && <div className="text-xs text-center font-bold">+{dayItems.length - 2} more</div>}
                      </div>
                    )}
                  </motion.div>
                );
  ...
  const filteredStickyNotes = useMemo(() => {
    return stickyNotes.filter((note) => note.title.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex flex-col lg:flex-row h-full pt-20 px-4">
        ...
        <div className="w-full lg:w-1/2 pl-2 bg-white dark:bg-gray-800 overflow-auto lg:block hidden">
          ...
          <div className="flex flex-col lg:flex-row mb-4 space-y-2 lg:space-y-0 lg:space-x-2">
            <Input type="text" value={newStickyNote} onChange={(e) => setNewStickyNote(e.target.value)} placeholder="タイトルを入力" className="flex-grow" />
            <Button onClick={addStickyNote} className="w-full lg:w-auto">
              追加
            </Button>
          </div>
          <div className="mb-4">
            <Input type="text" placeholder="付箋を検索..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full" />
          </div>
        ...
        {isModalOpen && (
          ...
          >
            <motion.div
              ...
              initial={{ y: "100%" }}
              animate={{
                y: modalState === "minimized" ? "calc(100% - 40px)" : modalState === "partial" ? "calc(100% - 400px)" : "0%",
            ...
            >
              <div className="h-10 flex items-center justify-start cursor-pointer overflow-x-auto whitespace-nowrap px-4" onClick={handleStatusBarClick} onPointerDown={(e) => dragControls.start(e)}>
                <ChevronUp className="h-6 w-6 text-gray-400 flex-shrink-0" />
                <span className="ml-2 text-sm text-gray-600 dark:text-gray-300">{format(selectedDate, "yyyy年MM月dd日", { locale: ja })}の付箋</span>
                {filteredStickyNotes.length > 0 && <span className="ml-2 text-sm text-gray-600 dark:text-gray-300">（{filteredStickyNotes.length}件）</span>}
              </div>
              <div className="p-4 overflow-y-auto" style={{ maxHeight: "calc(100% - 40px)" }}>
                ...
                <div className="space-y-4">
                  <Input type="text" value={newStickyNote} onChange={(e) => setNewStickyNote(e.target.value)} placeholder="新しい付箋のタイトルを入力" className="w-full" />
