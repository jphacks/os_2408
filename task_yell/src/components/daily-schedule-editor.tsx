...
export default function DailyScheduleEditor({ date = new Date(), initialEvents = [], newEvent }: { date?: Date, initialEvents?: Event[], newEvent?: Event }) {
  ...
  const getEventStyle = (event: Event, index: number, overlappingCount: number) => {
  ...
  const renderEvents = () => {
    ...
    sortedEvents.forEach((event, index) => {
      const overlappingEvents = sortedEvents.filter(e => isOverlapping(event) && e.id !== event.id);
  ...
  return (
    <div className="container mx-auto p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      ...
      <div className="relative h-[1440px]"> {/* 1440px = 24 hours * 60px per hour */}
        {hours.map((hour) => (
          <div key={hour} className="absolute w-full h-[60px]" style={{ top: `${(hour / 24) * 100}%` }}>
      ...
      <Dialog open={isAddEventDialogOpen} onOpenChange={setIsAddEventDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingEvent ? '予定を編集' : '新しい予定を追加'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                id="event-start"
                type="time"
                value={editingEvent?.start ? format(editingEvent.start, 'HH:mm') : ''}
                onChange={(e) => {
                  if (editingEvent) {
            ...
            <div>
              ...
              <Input
                id="event-end"
                type="time"
                value={editingEvent?.end ? format(editingEvent.end, 'HH:mm') : ''}
                onChange={(e) => {
                  if (editingEvent) {
                    const [hours, minutes] = e.target.value.split(':').map(Number)
